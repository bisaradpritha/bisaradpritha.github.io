...
// Fires once, the first moment the scroll-linked hero reveal
// starts — see the CSS note on .hero-content.typing-active
// for why this needs to be tied to actual visibility rather
// than page load.
...
function updateHeroScroll() {
    ...
    if (!typingTriggered && introProgress > 0) {
...
// Also run once immediately — covers the case where a
// browser restores the previous scroll position on reload
// without necessarily firing a "scroll" event, which would
// otherwise leave the hero stuck at its hidden initial state.
updateHeroScroll();
